    ...
    <div className='car-card'>
      ...
      <div className='car-info'>
        ...
        <p className='car-price'>
          {car.price ? `${car.price.toLocaleString()} $` : 'Цена по запросу'}
        </p>
        <div className='card-actions' style={{ display: 'flex', gap: '10px', flexDirection: 'column' }}>
          <button className='btn' onClick={() => dispatch(addToGarage(car))}>
            В гараж
          </button>
          <Link to={`/catalog/${car.id}`} className='btn btn-secondary' style={{ textAlign: 'center' }}>
            Подробнее
          </Link>
        </div>
      </div>
